...
public class SqlExecMojo extends AbstractMojo {
     ...
     * means that no sorting will be performed. Refers to {@link #fileset} and
     * {@link #srcFiles}
     *
     * @since 1.1
     */
    ...
    /**
     * Output File Encoding. If not given, it will use
     ...
     *
     * @param keepformat
     *            {@code true} to keep the format {@code false} otherwise.
     */
     ...
     *
     * @param enable
     *            <code>true</code> to escape, otherwise <code>false</code>
     */
	...
	try {
		...
		scriptRunner.run("Sql-Maven-Plugin pre-execute script", preExecuteHookScript, context, null, "pre-execute", false);
	    }
		...
		if (postExecuteHookScript != null) {
		    scriptRunner.run("Sql-Maven-Plugin post-execute script", postExecuteHookScript, context, null, "post-execute", false);
		}
	    } catch (RunFailureException e) {
		throw new MojoExecutionException(e.getMessage(), e);
	    } catch (IOException e) {
	...
	try {
		...
		if (outputFile != null) {
		    ...
		    out = new PrintStream(new BufferedOutputStream(new FileOutputStream(outputFile.getAbsolutePath(), append)), true,
			    outputEncoding);
		}
     ...
     * specified. The calling method is responsible for closing the connection.
     *
	...
	if (FILE_SORTING_ASC.equalsIgnoreCase(orderFile)) {
	    ...
	    throw new IllegalArgumentException(orderFile + " is not a valid value for orderFile, only '" + FILE_SORTING_ASC + "' or '"
		    + FILE_SORTING_DSC + "'.");
	}
    }
